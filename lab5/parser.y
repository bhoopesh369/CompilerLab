...
Function : Type ID '('')'  CompoundStmt {
	...
	if (!(strcmp($2,"printf") && strcmp($2,"scanf") && strcmp($2,"getc") && strcmp($2,"gets") && strcmp($2,"getchar") && strcmp	($2,"puts") && strcmp($2,"putchar") && strcmp($2,"clearerr") && strcmp($2,"getw") && strcmp($2,"putw") && strcmp($2,"putc") && strcmp($2,"rewind") && strcmp($2,"sprint") && strcmp($2,"sscanf") && strcmp($2,"remove") && strcmp($2,"fflush")))
		printf("\nError : Type mismatch in redeclaration of %s : Line %d",$2,printline());
	else
	{
		insert($2,FUNCTION,yylineno);
		insert($2,$1,yylineno);
		g_addr+=4;
	}
	}
	...
	if (!(strcmp($2,"printf") && strcmp($2,"scanf") && strcmp($2,"getc") && strcmp($2,"gets") && strcmp($2,"getchar") && strcmp	($2,"puts") && strcmp($2,"putchar") && strcmp($2,"clearerr") && strcmp($2,"getw") && strcmp($2,"putw") && strcmp($2,"putc") && strcmp($2,"rewind") && strcmp($2,"sprint") && strcmp($2,"sscanf") && strcmp($2,"remove") && strcmp($2,"fflush")))
	{printf("\nError : Redeclaration of %s : Line %d",$2,printline());errc++;}
	else
	{
		insert($2,FUNCTION,yylineno);
		insert($2,$1,yylineno);
...
parameter : Type ID {plist[++k]=$1;insert($2,$1,yylineno);insertscope($2,i);}
	          ;
...
assignment1 : ID {push($1);} '=' {strcpy(st1[++top],"=");} E {codegen_assign();}
	...
	| ID ',' assignment1    {
					if(lookup($1))
						printf("\nUndeclared Variable %s : Line %d",$1,printline());
				}
...
function_call: ID '=' E '(' paralist ')'			//function call
			{
							...
							{ printf("\nError : Type Mismatch : Line %d",printline()); errc++;}
								if(!lookup($1))
								{
									...
									if(ch>0) { printf("\nError : Parameter Type Mistake or Function undeclared : Line %d",printline()); errc++;}
									l=-1;
								}
			}
			| ID '(' paralist ')'			//function call without assignment
			{
							...
							if(!lookup($1))
							{
								...
								if(ch>0) { printf("\nError : Parameter Type Mistake or Required Function undeclared : Line %d",printline()); errc++;}
								l=-1;
							}
							else {printf("\nUndeclared Function %s : Line %d",$1,printline());errc++;}
			}
			;
...
param : ID
				{
			                if(lookup($1))
				        	{printf("\nUndeclared Variable %s : Line %d",$1,printline());errc++;}
			                else
			                {
			                	int sct=returnscope($1,stack[top-1]);
...
Declaration : Type ID {push($2);} '=' {strcpy(st1[++top],"=");} E {codegen_assign();} ';'
	{
		...
		if(!lookup($2))
		{
			...
			if(currscope==previous_scope)
				printf("\nError : Redeclaration of %s : Line %d",$2,printline());
			else
			{
	...
	| assignment1 ';'  {
				if(!lookup($1))
				{
					...
					int scope=returnscope($1,currscope);
					if(!(scope<=currscope && end[scope]==0) || scope==0)
						printf("\nError : Variable %s out of scope : Line %d",$1,printline());
				}
				else
					printf("\nError : Undeclared Variable %s : Line %d",$1,printline());
				}
/*
	| Type ID '[' assignment ']' ';' {
						insert($2,ARRAY,yylineno);
						insert($2,$1,yylineno);
						g_addr+=4;
					}
*/
	...
	| STRUCT ID '{' Declaration '}' ';' {
						insert($2,STRUCT,yylineno);
						g_addr+=4;
						}
	| STRUCT ID ID ';' {
				insert($3,STRUCT_VAR,yylineno);
				g_addr+=4;
				}
	| error
	;
